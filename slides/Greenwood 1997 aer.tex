...
\begin{frame}[t]
  \vfill
  \begin{center}
    {\color{ModernRed}\rule{0.2\linewidth}{0.9pt}\par}
    \vspace{0.35cm}
    {\bfseries\fontsize{38}{46}\selectfont 一个洞见 \par}
    \vspace{0.3cm}
    {\color{ModernRed}\rule{0.2\linewidth}{0.9pt}\par}
  \end{center}
  \vfill
...
\begin{frame}{传统模型：难以解释现有事实}
  ...
  \begin{block}{现有事实}
    \begin{itemize}
      \item 设备（投资）相对价格上升（$r/p^c \downarrow$）
      \item 设备（投资）占比下降（$I^i/Y \uparrow$）
  ...
  \begin{block}{传统模型局限：假设处于平衡增长路径上恒定}
    \begin{itemize}
      \item 投资占产出比重不变（平衡增长路径）
            $$ Y_t = C_t + I_t \Rightarrow \eta \equiv \frac{I_t}{Y_t} $$
      \item 投资相对消费价格不变（资本边际收益 = 边际产出）
            $$ p_c = {p_e} \times \dfrac{\partial Y_t}{\partial K_{e,t}}  \Rightarrow \dfrac{p_c}{p_e} =  \dfrac{\partial Y_t}{\partial K_{e,t}}  $$
    \end{itemize}
  \end{block}
\end{frame}
...
\begin{frame}[t]
  ...
  \begin{center}
    {\color{ModernRed}\rule{0.2\linewidth}{0.9pt}\par}
    \vspace{0.35cm}
    {\bfseries\fontsize{38}{46}\selectfont 如何解释？\par}
    \vspace{0.3cm}
    {\color{ModernRed}\rule{0.2\linewidth}{0.9pt}\par}
  \end{center}
  \vfill
...
\begin{frame}
  \begin{itemize}
    % \large
    \item 我们的一些解释：首先明确，\textbf{供给侧层面}的\textbf{价格冲击}
  \end{itemize}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.31\linewidth}
      \centering
      \includegraphics[width=\linewidth]{\detokenize{image/军器民用.png}}
      \vspace{0.2em}
      \parbox{\linewidth}{\centering\bfseries 军器民用：产业技术进步}
    \end{column}

    \begin{column}{0.31\linewidth}
      \centering
      \includegraphics[width=\linewidth]{\detokenize{image/管制放松.png}}
      \vspace{0.2em}
      \parbox{\linewidth}{\centering\bfseries 管制放松：国内竞争加剧}
    \end{column}

    \begin{column}{0.31\linewidth}
      \centering
      \includegraphics[width=\linewidth]{\detokenize{image/贸易自由.png}}
      \vspace{0.2em}
      \parbox{\linewidth}{\centering\bfseries 贸易自由：国外厂商进入}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}[t]{文中的解释：投资特定的技术进步}
  ...
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.48\linewidth}
      ...
      \begin{block}{}
        \begin{center}
          \large 投资特定的技术进步（Investment-Specific）
        \end{center}
      \end{block}
      \vspace{0.5em}
      \begin{block}{如何理解？}
...
\begin{frame}[t]
  ...
  \begin{center}
    {\color{ModernRed}\rule{0.2\linewidth}{0.9pt}\par}
    \vspace{0.35cm}
    {\bfseries\fontsize{38}{46}\selectfont 如何理解？\par}
    \vspace{0.3cm}
    {\color{ModernRed}\rule{0.2\linewidth}{0.9pt}\par}
  \end{center}
  \vfill
...
\begin{frame}{模型设定：为什么要和为什么能？}
  \begin{block}{为什么要？}
    ...
    \begin{itemize}
      \item 设备的相对价格下降：
            $$ p_c = {p_e} \cdot q  \times \dfrac{\partial Y_t}{\partial K_{e,t}} \Rightarrow \dfrac{p_e}{p_c} \downarrow \propto \dfrac{1}{q \uparrow} $$
      \item （有效）投资的相对占比提高：
            $$ \eta = \frac{i_e}{y} \Rightarrow \dfrac{i_e \cdot q \uparrow}{y} \uparrow $$
    \end{itemize}
  \end{block}
  ...
  \begin{block}{为什么能？}
    \begin{itemize}
      ...
      \item 理论支持：拆分后依然能使用复合生产函数，有对应模型。
    \end{itemize}
  \end{block}
...
\begin{frame}{模型总结}
  ...
  \begin{block}{消费者跨期效用最大化}
    \begin{equation*}
      \begin{aligned}
        ...
        s.t. \quad  c + \dfrac{k'_e}{q} + k'_s & = (1- \tau_k)\left[R_e k_e  + R_s  k_s \right] + (1-\tau_l)W l                        \\
                                               & + (1-\delta_e)\dfrac{k_e}{q} + (1-\delta_s)k_s + T                                    \\
  ...
  \begin{block}{厂商利润最大化}
    \begin{equation*}
      \max_{{k}_e, {k}_s, {l}} \pi_y = z F({k}_e, {k}_s, {l}) - R_e{k}_e - R_s {k}_s - W{l}
    \end{equation*}
  \end{block}
...
\begin{frame}{平衡增长的定义}
  ...
  \begin{block}{外生的技术进步：$\gamma_z$ 和 $\gamma_q$}
    ...
    \begin{equation*}
      z_t \equiv \left(\gamma_z\right)^t \qquad q_t \equiv \left(\gamma_q\right)^t
    \end{equation*}
  \end{block}
...
\begin{frame}{平衡增长的联系}
  ...
  \begin{block}{资本增速$g_s = g$，$g_e = g\gamma_q$}
    \begin{equation*}
      k_s = (1-\delta_s)k_s + i_s \Rightarrow g_s = (1-\delta_s) + \dfrac{i_s}{k_s} \Rightarrow g_s = g
    \end{equation*}
    因为,若 $g_s$ 为常数，则 $i_s/k_s$ 必为常数，$k_s$ 与 $i_s$ 同速率增长，同理有：
    \begin{equation*}
      k_e = (1-\delta_e)k_e + q\cdot i_e \Rightarrow  g_e = (1-\delta_e) + \dfrac{q i_e}{k_e} \Rightarrow g_e = g \gamma_q
    \end{equation*}
  \end{block}

  \begin{block}{增长速率的表示}
    将生产函数写成增速的表达形式，$g_y = \gamma_z g_s^{\alpha_s} g_e^{\alpha_e}$，化简有：
    \begin{equation*}
    ...
    \begin{column}{0.37\linewidth}
      \begin{block}{待估参数汇总（4/16）}
        \begin{itemize}
          ...
          \item 平衡路径：$ {\hat{c}}/{\hat y}, {\hat{i}_e}/{\hat y}, {\hat{i}_s}/{\hat y} $
          \item 要素份额：$ {\hat{k}_e}/{\hat y}, {\hat{k}_s}/{\hat y} $
        \end{itemize}
      \end{block}
    \end{column}
    \begin{column}{0.57\linewidth}
      ...
      \begin{enumerate}
        \item[1.] 技术进步（$\gamma$）：$\gamma_q = 1.032$
              \begin{equation*}
                \gamma_q = \dfrac{q_{t+1}}{q_t} =  \dfrac{\mathhighlight{p_{e,t}}}{\mathhighlight{p_{e,t+1}}}
              \end{equation*}
...
\begin{frame}{Figure 4：重新理解经济增长}
  ...
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.55\linewidth}
      \begin{figure}
        \centering
        \includegraphics[width=\linewidth]{image/fig4_productivity_measures.pdf}
        \vspace{-1em}
    ...
    \begin{column}{0.41\linewidth}
      ...
      \begin{block}{经济现实}
        \begin{itemize}
          \item 资本积累在一定程度上解释了增长 \\（$1.24\% \to 0.70\%$）
          \item 资本的分类对理解增长帮助不明显 \\ （$0.71\% \to 0.68\%$）
          \item 投资的技术进步是增长的重要因素 \\ （$0.68\% \to 0.39\%$）
        \end{itemize}
      \end{block}
    \end{column}
  \end{columns}
...
\begin{frame}{Figure 6：技术进步的估计误差}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.55\linewidth}
      \begin{equation}
        \text{预算约束：} c + p_e\cdot(i_e q) = zF(k_e,l)
      \end{equation}
      \begin{figure}
        \centering
        \includegraphics[width=\linewidth]{image/fig6.pdf}
        \vspace{-1em}
...
\begin{frame}{反事实估计}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.3\linewidth}
      \datapill{实际经济增长}{增长约 1.24\%}
    \end{column}
    \begin{column}{0.3\linewidth}
      \datapill{仅资本进步：$\gamma_z = 1$}{增长约 0.77\%}
    \end{column}
    \begin{column}{0.3\linewidth}
      \datapill{仅中性进步：$\gamma_e = 1$}{增长约 0.56\%}
    \end{column}
  \end{columns}
  \vspace{1em}
  \begin{block}{结论：理解增长动力}
    投资特定技术进步解释增长的 60\%，中性技术进步解释剩余的 40\% \\
    {
    \scriptsize
    注：$ z_t \equiv \left(\gamma_z\right)^t， q_t \equiv \left(\gamma_q\right)^t$，并且有 $ g = \gamma_z^{1/(1-\alpha_e - \alpha_s)} \gamma_q^{\alpha_e/(1-\alpha_e - \alpha_s)}$
    }
  \end{block}
  ...
  \begin{block}{重要的现实问题}
    \begin{enumerate}
      \item 长期增长从哪里来？
            \begin{itemize}
              % \scriptsize
              \item 传统理解：资本累计 + 劳动投入 + 技术进步
              \item 文章观点：三者并非相互独立，资本和劳动也可能有技术进步
            \end{itemize}
      \item 如何理解索洛悖论？
            \begin{itemize}
              % \scriptsize
              \item 传统理解：信息技术的发展，在 TFP 中难以统计
              \item 文章观点：需要对行业细分，的划分足够够细时才能观察到
            \end{itemize}
    \end{enumerate}
  \end{block}
\end{frame}

\begin{frame}{后续研究：什么文章在引用它？}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.68\linewidth}
      \vspace{-1em}
      \begin{figure}
        \centering
        \includegraphics[width=\linewidth,height=0.70\textheight,keepaspectratio]{\detokenize{image/GHK1997评价词云.png}}
        \caption{评价词云}
      \end{figure}
    \end{column}
        ...
        \begin{itemize}
          ...
          \item 技术进步的测算
                \begin{itemize}
                  ...
                  \item relative price of investment
                \end{itemize}
          \item 经济增长与波动
                \begin{itemize}
                  \item long-run growth, calibration benchmark
...
\begin{frame}{后续研究：什么文章在引用它？}
  ...
  \begin{block}{}
    \begin{enumerate}
      \item 技术进步的测算（35.8\%）
            \begin{itemize}
              \item 测量ISTC：开创了ISTC的研究
              \item 量化技术：将效率提高与价格降低联系起来，技术进步不再是残差
            \end{itemize}
      \item 经济增长与波动（32.5\%）
            \begin{itemize}
              \item 增长核算：是研究长期增长分解的重要基础，常被用于作为基准模型和参数参考
              \item 技术冲击：使得外部技术冲击可以被量化，被用于研究经济周期和经济波动
            \end{itemize}
      \item 经济结构的雏形（10.0\%）
            \begin{itemize}
              \item 将资本进行分类，并赋予价格、质量和技术等差异，已经有多部门的雏形
            \end{itemize}
    \end{enumerate}
  \end{block}
  ...
  % \vspace{0.4em}
  % \begin{alertblock}{今天仍然重要的问题}
  %   技术进步 embodied 在哪些资产中？如何被价格体系记录？又通过什么资本积累机制影响宏观结果？
  % \end{alertblock}
\end{frame}
...
\begin{frame}{核心启发}
  \begin{enumerate}
    \item 常规变量内部异质性
          \begin{itemize}
            \item 本文：将传统的 $k$，按更新速度不同划分为 $k_e$ 和 $k_s$
            \item 其它研究：
          \end{itemize}
    \item 冲击的非对称性影响
          \begin{itemize}
            \item 本文：技术冲击的影响，在不同资本之间存在差异（$q$）
            \item 其它研究：
          \end{itemize}
  \end{enumerate}
\end{frame}
