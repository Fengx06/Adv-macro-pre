...
\begin{frame}{两个重要的问题}
  ...
  % \begin{center}
  %   \small
  %   本文的洞察：\textbf{investment-specific technological change}
  % \end{center}
...
\begin{frame}[t]{文中的解释：投资专用技术进步}
  ...
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.48\linewidth}
      ...
      \begin{block}{}
        \begin{center}
          \large 投资偏向型的技术进步（investment-specific）
        \end{center}
      \end{block}
      \vspace{0.5em}
      \begin{enumerate}
...
\begin{frame}{核心结果}
  ...
  \begin{block}{结论：重新解释增长动力}
    投资偏向型技术解释增长的 60\%，中性技术增长解释剩余的 40\%
  \end{block}
\end{frame}
...
\begin{frame}
  \begin{figure}
    \centering
      \includegraphics[width=\linewidth,keepaspectratio]{\detokenize{image/文献综述.png}}
      \caption{后续研究}
  \end{figure}
...
%     \node[center] (ghk) at (-4.92,0) {
%       \textbf{Greenwood et al. (1997)}\\
%       \scriptsize
%       投资品相对价格下降——投资特定技术进步\\
%     };

%     \node[branch] (growth) at (2.02,2.18) {
%       \textbf{经济增长}\\
%       \scriptsize
%       \textbf{增长核算：}重新构造质量调整价格指数、引入中间品和投入产出结构、引入趋势断点和样本异质性、分析国家间增长率差异\\
%       Oulton (2007), Cummins--Violante (2002), Ngai--Samaniego (2009), Moura (2023)\\
%       \textbf{内生增长：}通过引入研发部分将投资特定技术进步内生化\\
%       Krusell (1998), Huffman (2007)
%     };

%     \node[branch] (cycle) at (2.02,0) {
%       \textbf{经济波动}\\
%       \scriptsize
%       \textbf{识别冲击：}将投资品相对价格用于识别经济周期中的投资品技术冲击，研究冲击对产出、劳动、投资、TFP波动的影响\\
%       Greenwood et al. (2000), Boileau (2002), Fisher (2006), Chen \& Wemy (2015)
%     };

%     \node[branch] (structure) at (2.02,-2.02) {
%       \textbf{结构转型}\\
%       \scriptsize
%       \textbf{投资结构：}不同产业部门的投资技术进步存在差异，从而推动产业结构转型\\
%       Herrendorf et al. (2021), Garcia-Santana et al. (2021), Guo et al. (2021)
%     };

%     \draw[arrow] (ghk) -- (growth);
%     \draw[arrow] (ghk) -- (cycle);
%     \draw[arrow] (ghk) -- (structure);
